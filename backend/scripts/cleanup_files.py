...
Run periodically (operator's note in TODO.md). ``GET /api/meta/files`` / the
...
from api.files import collect_file_state, key_file_id  # noqa: E402
from core.blank_node_handler import RFDB_BASE  # noqa: E402
from core.file_storage import (  # noqa: E402
    REGISTERED_PREFIX,
    STAGED_PREFIX,
    registered_key,
)
...
def reconcile(
    ...
    *,
    ...
    """
    state = collect_file_state(storage, oxigraph, extractor)
...
def main() -> int:
    ...
    from core.file_storage import build_storage
    from core.oxigraph_client import OxigraphClient
    from core.schema_extractor import SchemaExtractor

    storage = build_storage()
    oxigraph = OxigraphClient(str(settings.oxigraph_url), settings.data_graph_uri)
