...
predicate". ``GET /api/graph/node?id=<iri>`` then returns, for one node:
...
Reads are scoped to the configured named graph via ``OxigraphClient.from_clause``;
...
def _validate_iri(iri: str) -> None:
    ...
    if _IRI_UNSAFE.search(iri):
        raise HTTPException(status_code=400, detail=f"IRI contains unsafe characters: {iri}")
...
def _preferred_label(labels: list[Literal]) -> str | None:
...
def get_node(
    ...
    try:
        self_graph = oxigraph.construct(
            f"CONSTRUCT {{ <{id}> ?p ?o }} {oxigraph.from_clause()} WHERE {{ <{id}> ?p ?o }}"
        )
