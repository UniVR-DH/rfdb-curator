"""Unit tests for the meta routes: GET /api/meta/prefixes and /api/meta/graphs."""
...
def test_get_prefixes_returns_200_with_prefixes_key() -> None:
    ...
    res = TestClient(_make_meta_app()).get("/api/meta/prefixes")
...
def test_get_prefixes_covers_all_ttl_sources() -> None:
    ...
    prefixes = TestClient(_make_meta_app()).get("/api/meta/prefixes").json()["prefixes"]
...
def test_scan_ttl_prefixes_reads_header_and_stops_at_triples(tmp_path: Path) -> None:
    ...
    from core.prefixes import scan_ttl_prefixes
