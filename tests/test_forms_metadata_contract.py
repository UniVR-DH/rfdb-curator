"""Contract guard for the SHACL-extraction format exposed by /api/shapes and
/api/forms.
...
def test_shapes_carry_exactly_the_expected_keys() -> None:
    """Every /api/shapes entry has the documented key set — no more, no less."""
    shapes = _client().get("/api/shapes").json()
    assert shapes
    for shape in shapes:
...
def test_forms_envelope_and_field_keys_are_stable() -> None:
    """/api/forms returns {shape, fields} and every field has the documented keys."""
    body = _client().get("/api/forms", params={"shapeId": PLACE_SHAPE}).json()
...
def test_every_field_type_is_known() -> None:
    ...
    for shape in _client().get("/api/shapes").json():
...
def test_core_shapes_are_present() -> None:
    ...
    ids = {s["id"] for s in _client().get("/api/shapes").json()}
    for name in (
        "MusicalWorkShape",
        "ExpressionShape",
        "ManifestationShape",
