...
def build_storage(settings: S3Settings) -> FileStorage:
    ...
    ``settings`` is passed in rather than imported: this module is shared by the
    curator (writer) and dataexplorer (reader) services, which each own their own
    ``Settings`` subclass, so it cannot know which one to reach for — and a shared
    library importing a service's config would invert the dependency. Any object
    carrying the five ``s3_*`` attributes satisfies :class:`S3Settings`.
    """
